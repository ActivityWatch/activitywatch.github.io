...
if sys.version_info < (3, 5):
    sys.stdout.write("Python 3.5 or higher is required for gitstats")
    sys.exit(1)
...
def getpipeoutput(cmds, quiet=(not conf['verbose']), stream=sys.stdout):
    ...
    for x in cmds:
        ...
        p = subprocess.Popen(x, stdin=stdin, stdout=subprocess.PIPE, shell=True, cwd=os.getcwd())
        processes.append(p)
    ...
    def collect(self, dir):
        ...
        for line in lines:
            ...
            if ext not in conf['text_files_not_read']:
                ...
                print("Skipping file with ignored extension '{}': {}".format(ext, filename))
